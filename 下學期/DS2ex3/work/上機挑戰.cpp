...
class Hash{
	...
	void PrintUnsuccess() {
		...
		float num ;
		for ( int i = 0 ; i < hashtable.size() ; i++) {
			if ( hashtable[i].hasdata) // no data
	...
	void  Qsearch(char id[MAX_LEN], int key, int &probe, int i) { // 碰撞 
		probe++ ;
		if (hashtable[( key + i * i ) % hashsize].hasdata) { 
			if ( strcmp(id,student_list[hashtable[( key + i * i ) % hashsize].listvalue ].sid) == 0) {
				cout << "{ " << id << ", " <<  student_list[hashtable[( key + i * i ) % hashsize].listvalue ].sname <<
				", " << student_list[hashtable[( key + i * i ) % hashsize].listvalue].mean << " } is found after " <<
				probe << " probes." << endl ;
			} // if
			
			else 
				Qsearch(id, key, probe, i+1) ;
		} // if
	} //  Qsearch()
	...
	void SearchQuadratic() {
		...
		while ( strcmp(id, "0" ) != 0) {
			int key = 1 ;
			int len = strlen(id) ;		
			for ( int j = 0 ; j < len ; j ++) { // search key
				key *= id[j] ; 
				key = key % hashsize ; 
			} // for
			
			int probe = 0 ;
			Qsearch(id, key, probe, 0 ) ;
			cout << " cint" << endl;
			cin >> id ;
